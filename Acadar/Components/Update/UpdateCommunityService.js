...
export default function UpdateCommunityServce({user}){
    ...
    const addService = async() => {
        var body1 = await fetch(`http://localhost:5000/community/add`, {
            method: 'POST',
    ...
    const removeService = async (name) => {
        var body1 = await fetch(`http://localhost:5000/community/remove/${name}`, {
            method: 'DELETE',
            headers: { "Content-Type": "application/json" },
            credentials: "include"
        });
    ...
    const updateService = async (name) => {
        if(value === "") return;
        var body1 = await fetch(`http://localhost:5000/community/update/${name}/${field}`, {
            method: 'POST',
            headers: { "Content-Type": "application/json" },
            credentials: "include",
