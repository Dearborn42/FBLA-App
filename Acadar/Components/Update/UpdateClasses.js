...
export default function UpdateClasses({user}) {
    ...
    const addClass = async() => {
        if(year != ""){
            var body1 = await fetch(`http://172.233.131.223:5000/update/${year}`, {
                method: 'POST',
    ...
    const removeClass = async (year, name) => {
        var body1 = await fetch(`http://172.233.131.223:5000/update/${year}/${name}`, {
            method: 'DELETE',
            headers: { "Content-Type": "application/json" },
            credentials: "include"
        });
    ...
    const updateClass = async (year, name) => {
        ...
        var body1 = await fetch(`http://172.233.131.223:5000/classes/update/${year}/${name}/${field}`, {
            method: 'POST',
            headers: { "Content-Type": "application/json" },
            credentials: "include",
